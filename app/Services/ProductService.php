    ...
    public function createProduct($data, $request)
    {
        ...
        foreach ($users as $user) {
            if ($user->role == 'user') {
                ...
                if ($user->language == 'en')
                    $this->notificationService->sendNotification($user->fcm_token, $notification->name_en, $notification->description_en, $product->images[0]->image, '/main');
                else
                    $this->notificationService->sendNotification($user->fcm_token, $notification->name_ar, $notification->description_ar, $product->images[0]->image, '/main');
            }
        }
